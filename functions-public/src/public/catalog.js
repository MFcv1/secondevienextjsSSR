...
const readPublicCollection = async (collectionName, options = {}) => {
  ...
  if (cursorTimestamp) ref = ref.startAfter(cursorTimestamp);
  if (limitCount) ref = ref.limit(limitCount + 1);
...
const projectCardItem = (item) => {
  ...
  return {
    ...
    seoIndexable: item.seoIndexable,
...
const readSegmentedPublicCatalog = async (limitCount, scope = 'full', options = {}) => {
  ...
  const cached = limitedCatalogCache.get(cacheKey);
  if (cached && now - cached.cachedAt < CACHE_TTL_MS) return cached.catalog;
  ...
  if (!limitedInflightReads.has(cacheKey)) {
    limitedInflightReads.set(cacheKey, Promise.all(
      PUBLIC_COLLECTIONS.map(async (collectionName) => {
        const result = await readPublicCollection(collectionName, { limitCount, categories, cursor });
        return [collectionName, { items: projectCollectionItems(result.items, scope), nextCursor: result.nextCursor }];
      })
    )
      .then((entries) => {
