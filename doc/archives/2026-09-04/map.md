> ARCHIVE — photographie conservée le 2026-09-04 depuis `map.md`.
...
- `[F]`: Cloud Function;
- `[DB]`: Firestore;
- Performance admin: `AdminFunctionPerformance.jsx` (lazy, CSS module macOS)
  -> `GET /api/admin/function-metrics` (App Check + admin AAL2)
  -> `src/lib/server/functionMetrics.js` / `functionMetricsCache.mjs`
  -> `sys_function_metrics_cache/{24h|7d|30d}` [DB], serveur seul, trois snapshots
  -> `functionMetricsCore.mjs`, API Functions inventory + Cloud Monitoring [lecture].
  Aucun export Firebase Function ni collecte planifiee; permissions IAM de
  lecture et deploiement encore a realiser. Details: `_DOCS/admin/BACKOFFICE.md`.
- `[ST]`: Firebase Storage;
- `[EXT]`: service externe;
