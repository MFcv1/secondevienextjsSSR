...
import { ACTIVE_OBSERVABILITY_EXPORTS, EXPECTED_CURRENT_SOURCE_COUNT, PARALLEL_MIGRATION_EXPORTS, PENDING_OBSERVABILITY_EXPORTS, PENDING_RUNTIME_EXPORTS, extractLocalExports, runtimeFunctionNames } from '../scripts/functions-gen2-inventory.mjs';
...
test('G7 reconciles the inventory extractor through G6 and the 14 G7 exports', () => {
  ...
  assert.deepEqual(
    exports.filter(({ name }) => name.endsWith('Gen2') && !PARALLEL_MIGRATION_EXPORTS.has(name) && !PENDING_OBSERVABILITY_EXPORTS.has(name) && !PENDING_RUNTIME_EXPORTS.has(name) && !ACTIVE_OBSERVABILITY_EXPORTS.has(name)),
    []
  );
});
