...
test('Stripe accepts then response is lost: retry reuses one PI and attaches it', async () => {
    ...
    assert.equal(result.orderNumber, makeSagaOrder().orderNumber);
    assert.equal(result.shippingCents, makeSagaOrder().amounts.shippingCents);
...
test('crash after attach returns the attached PI on retry without creation', async () => {
    ...
    assert.equal(retry.orderNumber, makeSagaOrder().orderNumber);
    assert.equal(retry.shippingCents, makeSagaOrder().amounts.shippingCents);
