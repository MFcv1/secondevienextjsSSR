...
test('ile de version ecoute un seul document uniquement dans les onglets visibles et sans polling', () => {
  ...
  assert.match(island, /lastRefresh/);
  assert.match(island, /fetchCatalogWithRetry/);
  assert.match(liveGallery, /api\/catalog\?scope=cards&limit=48/);
  assert.match(liveGallery, /isVersionAtLeast/);
  assert.match(liveGallery, /catalogVersionChannel.subscribe/);
