...
const FilterForm = ({
  ...
  return (
    <form action={action} method="get" data-category-filter-form className="space-y-1">
      ...
      {hasActiveFilters ? (
        variant === 'mobile' ? (
          <a href={resetHref} className={`mt-4 inline-block text-[12px] underline underline-offset-4 ${darkMode ? 'text-stone-400' : 'text-[#9C8268]'}`}>
            R\u00e9initialiser
          </a>
        ) : (
          <a href={resetHref} className={`mt-4 block w-full rounded-lg py-2.5 text-center text-[11px] font-bold uppercase tracking-widest transition-colors ${darkMode ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-stone-100 text-stone-700 hover:bg-stone-200'}`}>
...
export default function CategoryServerView({
  ...
  const state = getCategoryQueryState(searchParams, filterOptions);
            ...
            <div className="mb-4 flex items-center justify-between gap-3 md:mb-0">
              ...
              <div className="flex items-center gap-3">
                <div className={`hidden items-center overflow-hidden rounded-lg border md:flex ${darkMode ? 'border-stone-700' : 'border-stone-200'}`}>
                  <a href={buildCategoryHref(categoryId, state, { viewMode: 'grid' })} className={`p-2 transition-colors ${state.viewMode === 'grid' ? (darkMode ? 'bg-white/10 text-white' : 'bg-stone-100 text-stone-900') : (darkMode ? 'text-stone-500 hover:text-stone-300' : 'text-stone-400 hover:text-stone-600')}`} aria-label="Vue grille">
                    <Grid3X3 size={16} />
                  </a>
                  <a href={buildCategoryHref(categoryId, state, { viewMode: 'list' })} className={`p-2 transition-colors ${state.viewMode === 'list' ? (darkMode ? 'bg-white/10 text-white' : 'bg-stone-100 text-stone-900') : (darkMode ? 'text-stone-500 hover:text-stone-300' : 'text-stone-400 hover:text-stone-600')}`} aria-label="Vue liste">
                    <List size={16} />
                  </a>
                </div>
                ...
                <div className={`flex items-center gap-1 md:hidden ${darkMode ? 'text-stone-300' : 'text-stone-800'}`}>
                  <a href={buildCategoryHref(categoryId, state, { mobileViewMode: 'grid' })} className={`flex h-9 w-9 items-center justify-center rounded-full transition-colors ${state.mobileViewMode === 'grid' ? (darkMode ? 'bg-stone-100 text-stone-950' : 'bg-stone-950 text-white') : (darkMode ? 'bg-white/10' : 'bg-stone-100')}`} aria-label="Vue grille">
                    <LayoutGrid size={16} strokeWidth={1.6} />
                  </a>
                  <a href={buildCategoryHref(categoryId, state, { mobileViewMode: 'list' })} className={`flex h-9 w-9 items-center justify-center rounded-full transition-colors ${state.mobileViewMode === 'list' ? (darkMode ? 'bg-stone-100 text-stone-950' : 'bg-stone-950 text-white') : (darkMode ? 'bg-white/10' : 'bg-stone-100')}`} aria-label="Vue liste">
            ...
            <div className="min-w-0 flex-1">
              {filteredItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  {hasActiveFilters ? (
                    <a href={categoryHref} className={`rounded-full border px-6 py-2.5 text-[10px] font-bold uppercase tracking-widest transition-colors ${darkMode ? 'border-stone-700 text-stone-300 hover:border-stone-400' : 'border-stone-300 text-stone-600 hover:border-stone-500'}`}>
                      R\u00e9initialiser
                    </a>
                  ) : null}
                </div>
                ...
                <>
                  <div className={`${state.mobileViewMode === 'grid' ? 'grid grid-cols-2 gap-4' : 'space-y-2.5'} md:hidden`}>
                    {filteredItems.map((item, index) => (
                      state.mobileViewMode === 'grid'
                        ? (
                          <div key={item.id} className="product-card-wrap relative">
                            ...
                            <CategoryProductCard item={item} layoutMode="grid" compact priority={index < 2} />
                          </div>
                        )
                  ...
                  <div className={`hidden gap-[24px] md:grid ${state.viewMode === 'grid' ? 'md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
                    {filteredItems.map((item, index) => (
                      <div key={item.id} className="product-card-wrap relative">
                        {isNewProduct(item) ? (
                        ...
                        <CategoryProductCard item={item} layoutMode={state.viewMode} compact priority={index < 2} />
                      </div>
                    ))}
                  </div>
                </>
