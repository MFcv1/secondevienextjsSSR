...
export function classifyAnalyticsCallableError(error) {
    ...
    if (code.includes('failed-precondition') && (message.includes('identit') || message.includes('passkey') || message.includes('google') || message.includes('session admin trop ancienne'))) return { kind: 'reauthentication-required', title: 'Confirmation administrateur à renouveler', detail: 'Reconnectez-vous avec votre passkey ou Google, puis actualisez les sessions et l’activité directe.' };
