...
const LINES = [
    ...
    ['waiting', 'En cours', 'bg-indigo-500'],
    ['done', 'Terminées', 'bg-emerald-500'],
];
...
export default function OrdersOverviewPanel({ darkMode = false, onSelectSegment, segment, summary }) {
    return (
        <div className="flex h-full min-h-0 flex-col justify-center px-6 py-8">
            <p className={`text-[8px] font-extrabold uppercase tracking-[0.14em] ${mutedTextClass(darkMode)}`}>
                À traiter
            </p>
            ...
            <p className={`mt-1.5 text-[11px] ${darkMode ? 'text-stone-500' : 'text-stone-400'}`}>
                {summary.todo === 0
                    ...
                    : `${formatPrice(summary.todoAmount)} en attente d’une action de votre part.`}
            </p>
            ...
            <div className={`mt-5 divide-y rounded-[18px] px-3 ring-1 ${insetClass(darkMode)} ${darkMode ? 'divide-white/10' : 'divide-black/[0.055]'}`}>
                {LINES.map(([id, label, color]) => (
                    ...
                    >
                        <span className={`flex items-center gap-2 text-[10px] font-semibold ${segment === id ? (darkMode ? 'text-white' : 'text-stone-950') : 'text-stone-500'}`}>
            ...
            <div className={`mt-5 rounded-[18px] px-4 py-3.5 ring-1 ${insetClass(darkMode)}`}>
                <p className={`text-[8px] font-extrabold uppercase tracking-[0.14em] ${mutedTextClass(darkMode)}`}>Encaissé</p>
                <p className="mt-1.5 text-[17px] font-extrabold tabular-nums tracking-[-0.03em]">
                    {formatPrice(summary.collectedAmount)}
                </p>
                <p className={`mt-1 text-[9.5px] leading-4 ${darkMode ? 'text-stone-600' : 'text-stone-400'}`}>
                    Calculé sur les {summary.total} commandes chargées, remboursements non déduits.
                </p>
            </div>

            <p className={`mt-auto pt-6 text-[9.5px] leading-4 ${darkMode ? 'text-stone-600' : 'text-stone-400'}`}>
