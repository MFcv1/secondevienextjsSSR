...
const APPROVAL = 'G5_CREATE_AUTH_SESSION_RUNTIME';
const SERVICE_ACCOUNT_ID = 'auth-session-runtime';
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!token.startsWith('--')) fail(`G5_IAM_ARGUMENT_INVALID:${token}`);
    const key = token.slice(2);
    const value = argv[index + 1];
    if (!value || value.startsWith('--')) fail(`G5_IAM_VALUE_MISSING:${key}`);
    args[key] = value;
    index += 1;
  }
  return args;
...
function run(command, args, { allowFailure = false } = {}) {
  ...
  if (result.error) fail(`G5_IAM_COMMAND_ERROR:${command}`);
  if (result.status !== 0 && !allowFailure) fail(`G5_IAM_COMMAND_FAILED:${command}:${args[0] || ''}`);
  return result;
}
...
function serviceAccountExists() {
  ...
  fail('G5_IAM_SERVICE_ACCOUNT_STATE_UNKNOWN');
}

async function main() {
  const args = parseArgs(process.argv.slice(2));
  if (args.project !== PROJECT || args.env !== ENVIRONMENT) fail('G5_IAM_TARGET_INVALID');
  const head = run('git', ['rev-parse', 'HEAD']).stdout.trim();
  if (!/^[0-9a-f]{40}$/.test(args.commit || '') || args.commit !== head) fail('G5_IAM_COMMIT_MISMATCH');
  if (args.execute && args.approval !== APPROVAL) fail('G5_IAM_APPROVAL_INVALID');

  const actualProject = run('gcloud', [
    'projects', 'describe', PROJECT, `--project=${PROJECT}`, '--format=value(projectId)',
  ]).stdout.trim();
  if (actualProject !== PROJECT) fail('G5_IAM_EFFECTIVE_PROJECT_MISMATCH');
  ...
  if (!existedBefore && args.execute) {
    run('gcloud', [
      ...
      `--project=${PROJECT}`,
      '--display-name=Functions Gen2 auth session runtime',
      '--description=Least privilege runtime for sandbox authentication session logging',
      '--quiet',
    ]);
  }
  ...
  if (args.execute && !ready) fail('G5_IAM_POSTCONDITION_FAILED');
