...
const LOG_DIR = path.join(ROOT_DIR, 'logs', 'card-thumbs');
const CACHE_CONTROL = 'public, max-age=31536000, immutable';
const PREVIEW_LIMIT = 25;
const CARD_THUMB_SPECS = [
...
function printHelp() {
    console.log(`
Backfill small product card WebP variants.

Dry-run is the default. Commit mode requires --confirm=CARD_THUMBS.
The script only creates or updates imageVariants[index].thumb320/thumb384.
It never deletes Storage files and never changes thumb/card/detailFast/medium/large/full.

Usage:
  node scripts/backfill-product-image-card-thumbs.cjs --dry-run --published-only --env=sandbox
  node scripts/backfill-product-image-card-thumbs.cjs --commit --confirm=CARD_THUMBS --published-only --env=sandbox
...
Options:
  ...
  --force                       Regenerate card thumbnails even if already present.
  --allow-production            Required with --commit --env=production.

Generated variants:
  thumb320 320px WebP quality 73.
  thumb384 384px WebP quality 74.
`);
}
...
function isProductionTarget(runtime) {
    return runtime.envName === 'production' || /prod/i.test(runtime.projectId);
}
...
function resolveCredential(options) {
    ...
    if (fs.existsSync(requestedPath)) {
        return {
            credential: admin.credential.cert(require(requestedPath)),
...
function buildVariantPath(collectionName, docId, index, source, spec) {
    ...
    return `${collectionName}/${spec.folder}/card_thumb_${safeDocId}_${index}_${spec.key}_${hash}.webp`;
}
...
async function createCardThumbBuffer(sourceBuffer, sharp, spec) {
...
async function uploadCardThumb(bucket, targetPath, variant, source, spec) {
    ...
    if (!exists) {
        ...
        await file.save(variant.buffer, {
            ...
            metadata: {
                ...
                metadata: {
                    firebaseStorageDownloadTokens: token,
                    generatedBy: 'scripts/backfill-product-image-card-thumbs.cjs',
...
async function describeDocNeed(docSnap, collectionName, runtime, bucket, options) {
    ...
    const images = getProductImages(data);
    const needs = [];
    const existingCounts = Object.fromEntries(CARD_THUMB_SPECS.map((spec) => [spec.key, 0]));

    for (let index = 0; index < images.length; index += 1) {
        const existingVariant = getVariantSlot(data, index);
        const missingSpecs = CARD_THUMB_SPECS.filter((spec) => {
            if (existingVariant[spec.key] && !options.force) {
                existingCounts[spec.key] += 1;
                return false;
            }
            return true;
        });
        if (missingSpecs.length === 0) continue;
...
async function scan(runtime, firebase, options) {
    ...
    for (const collectionName of runtime.collections) {
        ...
        for (const docSnap of docs) {
            ...
            stats.existingThumb320 += docNeed.existingCounts.thumb320;
            stats.existingThumb384 += docNeed.existingCounts.thumb384;
            ...
            stats.missingThumbSlots += missingCount;
            ...
            addPreview(previews, {
                ...
                needs: docNeed.needs.map((need) => ({
                    ...
                    variants: need.variants.map((entry) => ({
                        key: entry.spec.key,
            ...
            try {
                ...
                for (const need of docNeed.needs) {
                    ...
                    for (const { spec, targetPath } of need.variants) {
                        const variant = await createCardThumbBuffer(sourceBuffer, sharp, spec);
                        const uploadResult = await uploadCardThumb(firebase.bucket, targetPath, variant, need.source, spec);
                        ...
                        variantChanges.push({
                            ...
                            bytes: variant.bytes,
                            previousUrl: need.existingVariant[spec.key] || '',
                        });
...
function writeRunLog(runtime, options, result) {
    ...
    const payload = {
        ...
        script: 'scripts/backfill-product-image-card-thumbs.cjs',
...
function printSummary(runtime, firebase, options, result, logPath) {
    ...
    console.log(`Existing thumb320: ${stats.existingThumb320}`);
    console.log(`Existing thumb384: ${stats.existingThumb384}`);
    console.log(`Missing card thumb slots: ${stats.missingThumbSlots}`);
    ...
    if (previews.length > 0) {
        ...
        for (const item of previews) {
            console.log(`- ${item.collectionName}/${item.id} "${item.name}"`);
            for (const need of item.needs) {
                const variants = need.variants.map((variant) => variant.key).join(',');
                console.log(`  image[${need.index}] ${need.sourceType} ${need.sourceBytes || '?'} bytes -> ${variants}`);
            }
        }
    }
    ...
    if (options.dryRun) {
        console.log('\nNo writes performed. Re-run with --commit --confirm=CARD_THUMBS to upload card thumbnails and update Firestore.');
    }
}
...
async function main() {
    ...
    if (options.commit && options.confirm !== 'CARD_THUMBS') {
        throw new Error('Commit mode requires --confirm=CARD_THUMBS.');
    }
