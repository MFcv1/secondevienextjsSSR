...
const targetUrl = `${baseUrl}/a-propos`;
const shouldAssert = argv.has('assert');
const outDir = path.resolve(String(argv.get('out') || 'logs/about-direct-audit'));
...
const runMode = async (browser, mode) => {
  ...
  await page.waitForSelector('[data-ssr-about]', { state: 'attached', timeout: 30_000 });
  ...
  const dom = await page.evaluate(() => ({
    ...
    hasSsrAbout: Boolean(document.querySelector('[data-ssr-about]')),
...
for (const result of results) {
  add(`${result.mode}: SSR about marker is present`, result.dom.hasSsrAbout === true, result.dom);
  ...
  add(`${result.mode}: about H1 is present`, result.dom.h1.length > 0, result.dom);
  add(`${result.mode}: quote and gallery links are stable`, result.dom.hasQuoteLink === true && result.dom.hasGalleryLink === true, result.dom);
...
if (shouldAssert && !summary.assertions.passed) {
  console.error('Direct about audit assertions failed.');
  process.exit(1);
}
