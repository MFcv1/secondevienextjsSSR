...
const evaluateSnapshot = async (page, name) => {
  return page.evaluate((snapshotName) => {
    ...
    return {
      ...
      productCardImagesLoaded: productCards.filter((node) => node.querySelector('[data-product-media-state="ready"]')).length,
      productCardImagesPendingReveal: productCards.filter((node) => node.querySelector('[data-product-media-state="loading"], [data-product-media-state="decoding"]')).length,
