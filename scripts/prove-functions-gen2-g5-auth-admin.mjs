...
const OWNER_EMAIL = 'loa.gto15@gmail.com';
...
const secret = spawnSync('gcloud', [
  'secrets', 'versions', 'access', '3',
  '--secret=SUPER_ADMIN_EMAIL',
  `--project=${PROJECT}`,
  '--out-file=-',
], { encoding: null, stdio: ['ignore', 'pipe', 'pipe'] });
if (secret.status !== 0) fail('G5_AUTH_ADMIN_PROOF_OWNER_SECRET_UNREADABLE');
if (!Buffer.isBuffer(secret.stdout) || !secret.stdout.equals(Buffer.from(OWNER_EMAIL, 'utf8'))) {
  fail('G5_AUTH_ADMIN_PROOF_OWNER_SECRET_BYTES_MISMATCH');
}
...
if (
  ownerDecoded.email !== OWNER_EMAIL || ownerDecoded.email_verified !== true
