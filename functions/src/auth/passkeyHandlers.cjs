...
// Shared business logic: no deployment wrapper, SDK initialization or network on import.
function createPasskeyHandlers({
    admin, HttpsError, webauthn, getSiteUrl, getRateLimitClientIp,
    authorizePasskeyRegistration, createPasskeyTimer, logFunctionPerf
}) {
const functions = { https: { HttpsError } };
const { generateRegistrationOptions, verifyRegistrationResponse,
    generateAuthenticationOptions, verifyAuthenticationResponse } = webauthn;
...
function createPasskeyRegistrationAuthorizer({ HttpsError, readAccess, authorizeAdmin }) {
    return async (context) => {
        if (!context.auth?.uid) throw new HttpsError('unauthenticated', 'Connexion requise.');
        const token = context.auth.token || {};
        const access = await readAccess(context.auth.uid);
        // Check both the registry and claims: a token can predate a role grant.
...
module.exports = { createPasskeyHandlers, createPasskeyRegistrationAuthorizer };
