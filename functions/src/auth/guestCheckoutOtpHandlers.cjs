...
function createGuestCheckoutOtpHandlers({ admin, HttpsError, getRateLimitClientIp, OTP_HMAC_SECRET, getSiteUrl, timestampFromNow, SYSTEM_DOC_RETENTION_DAYS, getTransactionalEmailRuntime, logFunctionPerf }) {
...
const sendGuestCheckoutOtpHandler = async (data, context) => {
    ...
    const emailHash = sha256(email);
        let emailRuntime;
        try {
            emailRuntime = await getTransactionalEmailRuntime();
        } catch (error) {
...
return { sendGuestCheckoutOtpHandler, verifyGuestCheckoutOtpHandler, assertGuestCheckoutOtpVerified, normalizeGuestCheckoutEmail: normalizeEmail };
}
module.exports = { createGuestCheckoutOtpHandlers };
