...
- Fait 2026-05-25: `/produit/[slugOrId]` ne monte plus `ClientApp defer` pour l'experience publique directe. `src/app.jsx`, le shell galerie, le hero, les categories galerie et le footer differe ne sont plus charges sur une visite directe produit.
- Fait 2026-05-29: la route produit directe ne repose plus sur `ProductPageExperience` ni sur `ProductDetailRouteExperience`. Elle rend `ProductDetailServerView` cote serveur pour la structure visible produit, puis hydrate seulement `ProductDetailShellIsland` pour les medias/zoom/swipe/mobile et `ProductDetailActionsIsland` pour panier/favori. `ArchitecturalProductDetail` reste uniquement pour le detail ouvert depuis la galerie legacy.
- Fait 2026-05-30: les cartes produit publiques suivent directement leur lien `/produit/...` au lieu d'intercepter le clic pour monter l'overlay detail legacy. Le chemin galerie -> produit et le refresh dur utilisent donc la meme route produit Next native.
- Mesure produit direct apres ilots natifs: `npm run perf:product-direct` OK sur desktop/mobile CPU x4, 0 asset galerie/home, 0 marqueur SPA legacy, 0 shell galerie, 0 preview SSR alternative, variantes `thumb` + `medium` avant zoom. `perf:budget` mesure la route produit Next a `111.35 kB` JS gzip initial; le build indique `/produit/[slugOrId]` a `9.84 kB` de route et `113 kB First Load JS`.
- Prochaine etape N4: appliquer le meme decoupage a la galerie publique. Le markup stable des sections visibles doit pouvoir exister sans monter tout `src/app.jsx`/`Router.jsx`; wishlist, panier, admin, recherche, carousels et animations complexes doivent se brancher progressivement apres interaction, idle ou proximite viewport.
