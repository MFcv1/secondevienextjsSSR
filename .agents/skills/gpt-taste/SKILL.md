...
- **Premium Loading Bars:** Loading or route-transition bars must feel like a cinematic passage indicator, not a default spinner. Prefer a 1-2px hairline anchored to the top or bottom edge, driven by `transform: scaleX()` with `transform-origin: left`, a brief specular highlight, and an immediate fade once the destination is ready. Never use chunky progress bars, fake percentages, looping loaders over final content, or layout-affecting width animation.
