...
Fermeture G5-A2: `logUserConnectionGen2` partage exactement le handler Gen1,
conserve App Check, CPU 167m, 256 MiB, 60 s, concurrence/max 1 et min 0. Le
runtime `auth-session-runtime` porte uniquement Firestore, logs et service
usage, sans cle utilisateur ni secret. La revision
`loguserconnectiongen2-00001-fab` est ACTIVE; le refus sans Auth/App Check est
401 et l'appel admin positif est 200. Le rollout
`g5-a2-cutover-20260818-001` sert `build-2026-08-18-001`: ancien onglet `005`
reste rendu, `/` et `/admin` retournent 200, `lastLoginAt` est mis a jour, les
erreurs Gen2 et les nouveaux appels Gen1 valent zero. Gates G5: 7/7; manifeste
de fermeture: `functions-gen2-g5-log-user-connection-rollout.json`. Rollback
exact `005`; conserver Gen1, endpoint, IAM, source et donnees jusqu'a G12-A.
Prochaine cible unique: `ensureAdminAccessRegistryGen2`.

Fermeture G5-A3: `ensureAdminAccessRegistryGen2` partage exactement le handler
Gen1 et conserve App Check/AAL2. Le runtime `auth-registry-runtime` porte
Firestore, logs, service usage et l'acces borne au seul secret
`SUPER_ADMIN_EMAIL:3`, sans cle utilisateur. La revision
`ensureadminaccessregistrygen2-00001-lak` est ACTIVE; refus sans Auth/App Check
401, appel admin 200 avec `migrated:false`. Le rollout
`g5-a3-cutover-20260818-001` sert `build-2026-08-18-002`: ancien onglet `001`
reste rendu, routes 200, registre strictement inchange, zero erreur Gen2 et
zero nouvel appel Gen1. Gates G5: 10/10; manifeste de fermeture:
`functions-gen2-g5-ensure-admin-access-registry-rollout.json`. Rollback exact
`001`; conserver Gen1, endpoint, IAM, secret, source et donnees jusqu'a G12-A.
Prochaine cible unique: `sendGuestCheckoutOtpGen2`.

Fermeture G5-A4: `sendGuestCheckoutOtpGen2` partage exactement le handler
Gen1 et conserve App Check. Le runtime `auth-otp-email-runtime` porte
Firestore, logs, service usage et les acces bornes a `GMAIL_EMAIL:2`,
`GMAIL_PASSWORD:5`, `OTP_HMAC_SECRET:1` et `RESEND_API_KEY:1`, sans cle
utilisateur. La revision `sendguestcheckoutotpgen2-00001-neh` est ACTIVE;
refus sans Auth/App Check 401 puis un unique envoi sandbox HTTP 200, sans
lecture ni persistance du code. Le rollout `g5-a4-cutover-20260818-001` sert
`build-2026-08-18-003`: ancien onglet `002` authentifie et rendu, routes 200,
compteur admin 34, document de limitation stable apres l'envoi, zero erreur
Gen2 et zero nouvel appel Gen1. Gates G5: 13/13; manifeste de fermeture:
`functions-gen2-g5-send-guest-checkout-otp-rollout.json`. Rollback exact
`002`; conserver Gen1, endpoint, IAM, secrets, source et donnees jusqu'a G12-A.
Prochaine cible unique: `verifyGuestCheckoutOtpGen2`.

Fermeture G5-A5: `verifyGuestCheckoutOtpGen2` partage exactement le handler
Gen1, enforce App Check et epingle CPU 167m, 256 MiB, 60 s, concurrence/max 1,
runtime `auth-otp-verify-runtime` et `OTP_HMAC_SECRET:1`. La revision
`verifyguestcheckoutotpgen2-00001-wim` est ACTIVE; l'appel sans App Check est
refuse 401. Une seule verification OTP sandbox a reussi en HTTP 200: Gmail a
ete lu uniquement pour ce message, le code a ete chiffre RSA-OAEP entre les
processus puis efface, et ni code ni jeton checkout n'a ete affiche, journalise
ou persiste par le harnais. `build-2026-08-18-004` est READY et actif apres le
rollout `g5-a5-cutover-20260818-001`, un rollback reel reussi vers `003` puis
la reactivation finale `g5-a5-reapply-20260818-001`. L'ancien onglet `003` est
reste authentifie, `/` et `/admin` repondent 200, le compteur reste 34 et la
quiet-window a montre zero erreur Gen2, zero appel Gen1 et aucune mutation du
document OTP. Toutes les Gen1, IAM, secrets et donnees restent intacts.
Rollback exact `build-2026-08-18-003`; prochaine cible unique:
`sendCustomerLoginOtpGen2`. Inventaire: 167 local, 162 cloud, 139 Gen1,
23 Gen2, 8 schedulers, 2 queues et 7 Eventarc. Preuve machine:
`apphostingaudit/manifests/functions-gen2-g5-verify-guest-checkout-otp-rollout.json`.
...
G7-R est fermee le 2026-08-19 par
`apphostingaudit/manifests/functions-gen2-g7r.json`. Les neuf cibles cloud sont
Gen1 `ACTIVE` en `europe-west1`, Node 22, avec triggers, IAM et secrets `@1`
conformes; elles sont toutes classees `MIGRATE_GEN2`. Les cinq exports
Instagram directs ont chacun un appelant UI actif et un contrat distinct; ils
sont classes `MIGRATE_GEN2`, donc `m = 5`. Les deux secrets redirect
correspondent aux endpoints Gen1 attendus, mais le callback Instagram reste
...
et possibilites de rollback restent intacts. A cette fermeture G8, G9 n'etait
pas encore ouverte.
...
termine `ok` en 1,702 s sans backlog traite ni erreur. G9 a ete ouverte le
2026-08-22; son etat courant est consigne ci-dessous.
...
Etat d'execution du 2026-08-22: les 24 revisions Gen2 sont `ACTIVE`, les quatre
jobs Gen1 sont `PAUSED`, les quatre owners Gen2 `ENABLED`, et chaque scheduler
a prouve double invocation bornee, fence libere et rollback inverse. La probe
...
Fermeture G11-R du 2026-08-22: la recherche d'appelants executable et la
fenetre Cloud Logging du 2026-07-23 au 2026-08-22 donnent zero execution pour
les dix noms. `getUploadUrl`, les six purges/resets/GC globales,
`clearAllAffiliateClicks` et `clearAllSessions` sont `RETIRE_G12_A`:
absence d'appelant ou handler non rendu pour les sept premiers, et convergence
des deux purges analytics globales vers l'outil de retention borne.
`deleteSession` est la seule `MIGRATE_GEN2` (`d = 1`), car son appel cible par
session reste joignable dans `AdminAnalytics` et aucun doublon serveur ne couvre
ce besoin d'exploitation. Le manifeste
`manifests/functions-gen2-g11-r.json` porte les versions, appelants,
protections, doublons et raisons par cible. Inventaire inchange: 275 local / 272
cloud / 139 Gen1 / 133 Gen2. App Hosting reste `build-2026-08-22-001`; aucun
deploy, build, IAM, secret, retrait Gen1 ou acces aux donnees n'a eu lieu.

Apres cette fermeture, l'utilisateur a leve explicitement
`HOLD_G11_DESTRUCTIVE_PRECONDITIONS` pour G11-D sur le sandbox, tout en
interdisant toute destruction de donnees reelles, production et Stripe live.
